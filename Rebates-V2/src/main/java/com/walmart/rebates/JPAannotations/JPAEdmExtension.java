...
import com.walmart.rebates.annotations.SAPLineItem;
import com.walmart.rebates.annotations.Sap;
	...
	public void extendJPAEdmSchema(JPAEdmSchemaView view) {
		...
		for (EntityType entityType : edmSchema.getEntityTypes()) {
			for (Property property : entityType.getProperties()) {
				...
				if (label != null) {
					annotationAttributeList.add(new AnnotationAttribute().setName("semantics")
							.setNamespace(SAP_NAMESPACE)
							.setPrefix(SAP_PREFIX)
							.setName(LABEL).setText("aggregate"));
				}
			...
			if (field.getName().equals(((JPAEdmMappingImpl) property.getMapping()).getInternalName())) {
				if (field.getAnnotation(Sap.class) != null) {
				    ...
				    for (Entry<String, Object> memberValue : memberValues.entrySet()) {
				    	String aggAnnotation = String.valueOf(memberValue.getValue());
						if(aggAnnotation=="aggregation_role"){
							aggAnnotation = "aggregation-role";
						}
				    	result.add(new AnnotationAttribute()
								.setNamespace(SAP_NAMESPACE)
								.setPrefix(SAP_PREFIX)
								.setName(memberValue.getKey())
								.setText(aggAnnotation));				
					}
				}
			}
		}
